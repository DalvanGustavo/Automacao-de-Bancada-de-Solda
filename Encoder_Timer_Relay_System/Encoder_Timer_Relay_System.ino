...
const char* topic_status = "meu_esp32_c3_supermini/status"; // Novo!
...
// Variáveis para rastrear se algo mudou e avisar o site
int last_minutos = -1;
int last_segundos = -1;
int last_estadoLigado = -1;
int last_senhaCorreta = -1;
...
void desenharTela() {
  ...
  if(senhaCorreta == 0){
    ...
    if(estadoLigado == 0){
      if(estado == 0){ display.print("Editando: Minutos"); }
      else { display.print("Editando: Segundos"); }
    }else{
    ...
    int alturaLinha = 45;
    if(estadoLigado == 0){
      if(estado == 0) { display.drawLine(15, alturaLinha, 45, alturaLinha, SSD1306_WHITE); } 
      else { display.drawLine(70, alturaLinha, 100, alturaLinha, SSD1306_WHITE); }
    }
  }
  display.display();
}

// --- FUNÇÃO QUE AVISA O SITE SOBRE O STATUS ---
void checarStatusEEnviar() {
  // Só envia se algo realmente mudou (para não travar a internet)
  if (minutos != last_minutos || segundos != last_segundos || estadoLigado != last_estadoLigado || senhaCorreta != last_senhaCorreta) {
    
    last_minutos = minutos;
    last_segundos = segundos;
    last_estadoLigado = estadoLigado;
    last_senhaCorreta = senhaCorreta;
    
    if (client.connected()) {
      char msg[50];
      // Formato: STATUS <senhaCorreta> <ligado> <minutos> <segundos>
      sprintf(msg, "STATUS %d %d %d %d", senhaCorreta, estadoLigado, minutos, segundos);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (msg.startsWith("LIGAR")) {
    ...
    estadoLigado = 0; // APENAS DESLIGA (PAUSA). Não zera mais o tempo!
...
void conectarMQTT() {
  if (millis() - ultimoTentoMQTT > 10000) {
    if (client.connect("ESP32_C3_TimerClient")) {
      ...
      // Força enviar o status atual assim que conecta
      last_segundos = -1; 
    }
...
void loop() {
      ...
      if(senhaCorreta == 0){
        if(tempoPressionado >= 5000){
          if(senha[0] == senhaMestra[0] && senha[1] == senhaMestra[1] && senha[2] == senhaMestra[2] && senha[3] == senhaMestra[3]){
            ...
            senhaCorreta = 0; errouSenha = !errouSenha; estadoSenha = 0;
            for(int i =0; i < 4; i++){ senha[i] = 0; }
            desenharTela(); digitalWrite(buzzer, HIGH); delay(2000); digitalWrite(buzzer, LOW);
          }
          desenharTela();
        }
        ...
        if(tempoPressionado >= 10000){
          // BLOQUEAR FÍSICO
          senhaCorreta = 0; estadoLigado = 0; estadoSenha = 0;
          digitalWrite(led, LOW); digitalWrite(buzzer, LOW); digitalWrite(rele, LOW);
          for(int i =0; i < 4; i++){senha[i] = 0;}
          desenharTela();
        }
          ...
          // Aqui foi removido o reset do tempo. Ele apenas pausa.
          desenharTela();
        }
  ...
  if(senhaCorreta == 1){
    if(estadoLigado == 0){
      ...
      if(minutos == 0 && segundos == 0){
        // ZEROU O TEMPO! APENAS AQUI ELE VOLTA PARA 10 MINUTOS
        digitalWrite(rele, LOW); digitalWrite(led, LOW); digitalWrite(buzzer, LOW);
        estadoLigado = 0;
        minutos = 10;
        desenharTela();
      }
        ...
        if(minutos < 5){
          ...
          if(segundos == 59) digitalWrite(buzzer, HIGH); else digitalWrite(buzzer, LOW);
        } else {
  ...
  // No final de cada volta, verifica se algo mudou e avisa o Python/Site!
  checarStatusEEnviar();
}